...
LANGUAGES = ["Afrikaans", "Arabic", "Dutch", "French", "German", "Hindi", "Indonesian", "Italian", "Japanese", "Latvian", "Portuguese, Brazilian", "Romanian", "Russian", "Spanish", "Turkish", "Vietnamese","Lithuanian","Polish","Ukrainian"]
...
class Languages(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.translation_channel: discord.TextChannel = self.bot.get_channel(TRANSLATOR_CHANNEL)
    ...
    async def mention_translators(self, interaction: discord.Interaction, message: discord.Message) -> None:
        ...
        if channel.parent_id != 1446878843250938019:
            await interaction.response.send_message(
                "This message is not in a translator thread.", ephemeral=True
            )
            return
        ...
        proofreaders = PROOF_READERS.get(language, [])
